...
export function StealthCard({ children, className, hover = true }: StealthCardProps) {
  return (
    <div
      className={cn(
        "relative overflow-hidden rounded-sm border border-border bg-card backdrop-blur-xl",
        ...
        hover && "stealth-shimmer transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] hover:-translate-y-px hover:border-border hover:shadow-[0_4px_12px_rgba(0,0,0,0.08)] dark:hover:shadow-[0_4px_12px_rgba(0,0,0,0.4)]",
        className
      )}
    >
      {children}
