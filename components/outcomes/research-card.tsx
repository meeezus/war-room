...
export function ResearchCard({ data }: { data: OutcomeCard | null }) {
  if (!data) {
    return (
  ...
  return (
    <StealthCard className="p-4">
      <div className="flex items-center gap-2 mb-2" title="External intelligence — Twitter, arxiv, autoresearch, industry scanning">
      ...
      <p className="text-xl font-semibold text-blue-500 font-[family-name:var(--font-jetbrains-mono)] mb-2">
        {data.headline}
      </p>

      {isEmpty ? (
        <p className="text-sm text-muted-foreground font-[family-name:var(--font-space-grotesk)]">
          Research pipeline initializing — findings will appear once scanning tools are wired
        </p>
      ) : (
      ...
      {data.actionLabel && data.actionHref && (
        <Link
          ...
          className="inline-block text-xs text-blue-500 hover:text-blue-400 font-medium transition-colors"
        >
          {data.actionLabel}
        </Link>
      )}
