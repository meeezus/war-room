...
def create_mission(
    ...
    for step in steps:
        ...
        daimyo_id = DOMAIN_TO_DAIMYO.get(domain, "ed")
...
def run_pending() -> list[dict]:
    ...
    # 1. Query approved proposals without missions
    proposals_result = supabase.table("proposals").select("*").eq("status", "approved").execute()

    if not proposals_result.data:
        return []

    # Get existing mission proposal IDs to filter out already-processed proposals
    ...
    # Filter to only proposals without missions
    pending_proposals = [p for p in proposals_result.data if p["id"] not in existing_proposal_ids]

    created_missions = []

    # 2. For each pending proposal, decompose into steps and create mission
    for proposal in pending_proposals:
        # Simple K2.5 heuristic: research → code → review
        steps = [
            {
        ...
        # Assign primary daimyo based on proposal domain
        domain = proposal.get("domain", "engineering")
